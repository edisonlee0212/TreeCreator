...
void SorghumReconstruction::SorghumReconstructionSystem::ExportPlant(Entity plant, std::string path) const
{
	...
	if (of.is_open())
	{
		...
		auto mesh = EntityManager::GetPrivateComponent<MeshRenderer>(plant)->Mesh;
		ObjExportHelper(glm::vec3(0.0f), mesh, of, startIndex);
		
		EntityManager::ForEachChild(plant, [this, &of, &startIndex](Entity child)
			{
				auto lt = EntityManager::GetComponentData<Transform>(child).GetPosition();
				auto mesh = EntityManager::GetPrivateComponent<MeshRenderer>(child)->Mesh;
...
void SorghumReconstruction::SorghumReconstructionSystem::OnCreate()
{
	...
	auto textureDiffuseTruck = ResourceManager::LoadTexture(false, FileIO::GetResourcePath("Textures/brown.png"));
	...
	auto textureLeaf = ResourceManager::LoadTexture(false, FileIO::GetAssetFolderPath() + "Textures/leafSurfaceDark.jpg");
