	...
	if (of.is_open())
	{
		...
		for (auto treeEntity : trees)
		{
			...
			if(!mesh->GetVerticesUnsafe().empty() && !mesh->GetIndicesUnsafe().empty())
			{
				...
				o += 
					"["
				+ std::to_string(ltw.m_value[3].x) + ","
				+ std::to_string(ltw.m_value[3].z)
				+ "]" + "\n";
...
#pragma region Data collection
				...
				for (const auto& vertex : mesh->GetVerticesUnsafe()) {
					data += "v " + std::to_string(vertex.m_position.x + ltw.m_value[3].x)
						+ " " + std::to_string(-vertex.m_position.z + ltw.m_value[3].y)
						+ " " + std::to_string(vertex.m_position.y + ltw.m_value[3].z)
						+ "\n";
				}
