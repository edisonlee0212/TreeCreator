...
void TreeScene::ExportSceneAsOBJ(std::string filename)
{
	...
	if (of.is_open())
	{
		...
		for (auto treeEntity : trees)
		{
			...
			auto info = EntityManager::GetComponentData<TreeInfo>(treeEntity);
			
			if(!info.Vertices->empty() && !info.Indices->empty())
			{
				std::string header = "#Tree vertices: " + std::to_string(info.Vertices->size()) +", tris: " + std::to_string(info.Vertices->size() / 3);
				header += "\n";
...
#pragma region Data collection
				...
				for (auto i = 0; i < info.Indices->size() / 3; i++) {
					auto f1 = info.Indices->at(3l * i) + startIndex;
					auto f2 = info.Indices->at(3l * i + 1) + startIndex;
					auto f3 = info.Indices->at(3l * i + 2) + startIndex;
					data += "f " + std::to_string(f1)
						+ " " + std::to_string(f2)
						+ " " + std::to_string(f3)
						+ "\n";
				}
				startIndex += info.Vertices->size();
#pragma endregion
