...
std::string TreeCollectionGenerationSystem::_BaseStorePath = "./tree_perceptual/";
...
void TreeUtilities::TreeCollectionGenerationSystem::LateUpdate()
{
	...
	case TreeCollectionGenerationSystenStatus::CollectData:
		_DataCollectionSystem->_ImageCameraEntity.GetPrivateComponent<CameraComponent>()->StoreToPng(
			_StorePath + "image/" +
			std::string(7 - std::to_string(_Counter).length(), '0') + std::to_string(_Counter) + ".png"
			, 1024, 1024, true);
...
void TreeCollectionGenerationSystem::TakeSnapShot()
{
	...
	TreeManager::SerializeTreeGraph(_BaseStorePath + "snapshot/graph", trees->front());
	_DataCollectionSystem->_ImageCameraEntity.GetPrivateComponent<CameraComponent>()->StoreToPng(
